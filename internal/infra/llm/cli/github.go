...
func (p *copilotProvider) GetGenerateArgs(ctx context.Context, prompt string, assetPaths []string, runtime *providerruntime.SessionRuntime) ([]string, error) {
	// Ensure apps.json is present in runtime.ConfigDir so the copilot binary is authenticated
	destAppsPath := filepath.Join(runtime.ConfigDir, "apps.json")
	if _, err := os.Stat(destAppsPath); os.IsNotExist(err) {
		home, err := os.UserHomeDir()
		if err == nil {
			srcAppsPath := filepath.Join(home, ".config", "github-copilot", "apps.json")
			if _, err := os.Stat(srcAppsPath); err == nil {
				_ = os.Symlink(srcAppsPath, destAppsPath)
			}
		}
	}
