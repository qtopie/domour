...
func AttachmentsFromProto(items []*commonpb.Attachment) []shared.BrainAttachment {
	if len(items) == 0 {
		return nil
	}

	attachments := make([]shared.BrainAttachment, 0, len(items))
	for _, item := range items {
		if item == nil {
			continue
		}
		attachment := shared.BrainAttachment{
...
func BuildUserInputMessage(text string, attachments []shared.BrainAttachment) (*schema.Message, error) {
...
func attachmentToInputPart(attachment shared.BrainAttachment) (schema.MessageInputPart, error) {
...
func attachmentToInputImage(attachment shared.BrainAttachment, mimeType string) (*schema.MessageInputImage, error) {
...
func NormalizeAttachmentMIMEType(attachment shared.BrainAttachment) string {
...
func attachmentName(attachment shared.BrainAttachment) string {
