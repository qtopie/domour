...
### 2.2 小脑 (Cerebellum): 战术与逻辑编排层
- **职责**: 接收 大脑 (Cerebrum) 的决策，负责具体战术的编排与工具调度序列，决定“怎么做”(How to do)。
- **特性**: 确定性、过程协调。它通过 ReAct 循环或本地规划管理工具调用序列，并将观测结果 (Observation) 上报给间脑。
- **功能**: 编排执行 Tool/Skill，并对局部动作和模型输出结果进行初步校对（如 Simple 模式的纠偏反馈）。

小脑连接脑干，单向连接给间脑反馈。

### 2.3 脑干 (Brainstem / Motor): 生命基础与物理安全执行层
- **职责**: 维持生命运转的底层基础。负责具体物理系统调用（Shell、File I/O）、安全拦截 (Veto)、计算资源分配、状态持久化与事件总线，拥有执行的最终决定权，决定“在哪做”和“是否允许做”(Where & Allowed to do)。
- **特性**: 稳定性、物理控制、高安全性。脑干 (Motor) 作为最底层防线，有权否决（Veto）任何来自上层认知（Cerebrum/Cerebellum）的不安全指令。
- **安全设定**: 脑干实现底层安全屏障。
